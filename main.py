    ...
    with psycopg2.connect(database="1", user="postgres",
        ...
        with conn.cursor() as curs:
            ...
            print("Добавлен клиент id: ",
                  insert_client(curs, "Ольга", "Петрова", "olgapetrova@mail.ru"))
            print("Добавлен клиент id: ",
                  insert_client(curs, "Ирина", "Иванова",
                                "irinaivanova@mail.ru", 79005554433))
            print("Добавлен клиент id: ",
                  insert_client(curs, "Юлия", "Федорова",
                                "yuliafedorova@mail.ru", 79557773425))
            print("Добавлен клиент id: ",
                  insert_client(curs, "Иван", "Сорокин",
                                "ivansorokin@mail.ru", 79006783452))
            print("Добавлена клиент id: ",
                  insert_client(curs, "Артем", "Лапочкин",
                                "artemlapochkin@mail.ru"))
            ...
            print("Телефон добавлен клиенту id: ",
                  insert_tel(curs, 2, 79085674497))
            print("Телефон добавлен клиенту id: ",
                  insert_tel(curs, 1, 79607835647))
            ...
            print("Изменены данные клиента id: ",
                  update_client(curs, 4, "Игорь", None, 'igorsorokin@mail.ru'))
            # 5. Удаляем клиенту номер телефона
            print("Телефон удалён c номером: ",
                  delete_phone(curs, '79607835647'))
            print("Данные в таблицах")
            curs.execute("""
                SELECT c.id, c.name, c.lastname, c.email, p.number FROM clients c
                LEFT JOIN phonenumbers p ON c.id = p.client_id
            ...
            pprint(find_client(curs, 'Ирина'))

            print('Найденный клиент по email:')
            pprint(find_client(curs, None, None, 'artemlapochkin@mail.ru'))

            print('Найденный клиент по имени, фамилии и email:')
            pprint(find_client(curs, 'Юлия', 'Федорова',
                               'yuliafedorova@mail.ru'))

            print('Найденный клиент по имени, фамилии, телефону и email:')
            pprint(find_client(curs, 'Иван', 'Сорокин',
                               'ivansorokin@mail.ru', '79006783452'))

            print('Найденный клиент по имени, фамилии, телефону:')
            pprint(find_client(curs, None, None, None, '79557773425'))